...
<script>
			...
			for (var i = 0, j = params.result.length; i < j; i++) {
				...
				if (params.result[i].STEP == 0) {
					if (params.result[i].REGNT_ID == m_id) {
						tmpStr += "<li id='comment_grp_"+params.result[i].GRP_SEQNO+"' "+cls+"><p><em>"
								+ params.result[i].REGNT_ID + "</em>";
						tmpStr += "<span>" + params.result[i].REGNT_DTM
								+ "</span>";
						tmpStr += btn_bar;
						tmpStr += "<div class='wrap_txt_mod'> <textarea class='txtarea_mod' id='rpl_"+params.result[i].SEQ_NO+"'cols='100' rows='2'>"
								+ content
								+ "</textarea><button class='btn_reply_mod' id='mod_"+params.result[i].SEQ_NO+"'>수정</button> </div>"
						tmpStr += "<div class='cm_txt cm_"+ params.result[i].SEQ_NO+"'>"
								+ content + "</div>";
					...
					if (params.result[i].REGNT_ID == m_id) {
						...
						tmpStr += "<p><em>" + params.result[i].REGNT_ID
								+ "</em><span>" + params.result[i].REGNT_DTM
								+ "</span>";
						tmpStr += btn_bar;
						tmpStr += "<div class='wrap_txt_mod'> <textarea class='txtarea_mod' id='rpl_"+params.result[i].SEQ_NO+"'cols='100' rows='2'>"
								+ content
	...
	$(document)
			.on(
					...
					function() {
						...
						// "M" 버튼을 "C" 버튼으로 변경합니다.
						$(this).html(C).removeClass("btn_comment_mod")
								.addClass("btn_comment_cancel");
					});
	...
	$(document)
			.on(
					...
					function() {
						...
						$(this).html(M).removeClass("btn_comment_cancel")
								.addClass("btn_comment_mod");
					});
	...
	$(document).on(
			...
			function() {
				...
				var grp_seqno = $(this).parent().parent().parent().attr("id")
						.replace(/comment_grp_/gi, "");
				...
				<div class="baord">
					<div class="view">
						<div class="col01">
							...
							<p class="sm_txt">
								<span class="review_nm"></span>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<span
									class="review_regdtm">2023.05.13</span>
							</p>
							<p>
								조회수: <span class="view_cnt"></span>
							</p>
							<p class="txt_area review_contents"></p>
							<p class="tac">
								<button class="btn_like">
									<img class="btn_likeIcon" src="../../img/btn_like.png"
										alt="like"> <span class="btn_likeNum review_likeNum"></span>
								</button>
							</p>
						</div>
					</div>
					<div class="navigation clear">
						<a href="javascript:///" class="fl nv_prev">이전</a> <a
							href="../review/review.do">LIST</a> <a href="javascript:///"
							class="fr nv_next">다음</a>
					</div>

...
// ******************************************************************************************* 
	...
	function fn_listType_bind(params) {
		...
		if (params.result.length > 0) {
			...
			for (var i = 0, j = params.result.length; i < j; i++) {
				...
				tmpStr += "	<td>";
				tmpStr += "<a href='../notice/notice_view.do?seqno="
						+ params.result[i].SEQ_NO + "' id='sTitle'>"
						+ params.result[i].TITLE + "</a>";
				tmpStr += "	</td>";
				tmpStr += "	<td id='sView'>" + params.result[i].VIEW_CNT
						+ "</td>";
				tmpStr += "	<td id='sRegdate'>" + params.result[i].LIKE_CNT2
						+ "</td>";
				tmpStr += "	<td id='sView'>" + 0 + "</td>";
				tmpStr += "	<td id='sRegdate'>" + params.result[i].REGNT_DTM
						+ "</td>";
				tmpStr += "</tr>";
	...
	function fn_paging(params) {
		...
		tmpStr += "<a href='javascript://' class='prev' data-num='"
				+ (parseInt(_g_pageNo) - 1) + "'></a>";

		for (i = 1; i <= pageBlock; i++) {
			if (_g_pageNo == i) {
				tmpStr += "<a href='javascript://' class='paging_num on' data-num='"+i+"'>"
						+ i + "</a>";
			} else {
				tmpStr += "<a href='javascript://' class='paging_num' data-num='"+i+"'>"
						+ i + "</a>";
			}

		}

		tmpStr += "<a href='javascript://' class='next paging-btn' data-num='"
				+ (parseInt(_g_pageNo) + 1) + "'> </a>";
				...
				<div class="baord" data-aos="fade-up" data-aos-duration="1000">
					<div class="tb_top clear">
						<div class="left">
							...
							</select> <input type="text" class="search" placeholder="search">
							<button class="btn_search">
								<span class="hide">search</span>
							</button>
						</div>
						<div class="right btn_area">
							<button class="btn_blue" id="btn_write">
								작성하기 <i class="fa-solid fa-feather" style="color: #ffffff;"></i>
							</button>
						</div>
					</div>
